...
class SlideSlide(models.Model):
    ...
    source_type = fields.Selection([
        ...
        ('external', 'Retrieve from Link')],
        default='local_file', required=True)
    # generic
    url = fields.Char('External URL', help="URL of an external resource such as Google Drive, YouTube, or Vimeo")
    ...
    image_google_url = fields.Char('Image Link', related='url', readonly=False,
        help="Link of the external image resource")
    # content - documents
    ...
    slide_type = fields.Selection([
        ...
        ('local_audio', 'Uploaded Audio'),
        ('local_video', 'Uploaded Video'),
        ('youtube_video', 'YouTube Video'),
        ('google_drive_video', 'Google Drive Video'),
        ('vimeo_video', 'Vimeo Video'),
        ('cloudflare_stream_video', 'Cloudflare Stream Video'),
        ('external_video', 'External Iframe Video')],
    ...
    document_google_url = fields.Char('Document Link', related='url', readonly=False,
        help="Link of the external document resource")
    ...
    video_url = fields.Char('Video Link', related='url', readonly=False,
        help="Paste only the video or iframe URL. The iframe is generated by the website code.")
    ...
    nbr_audio = fields.Integer("Number of Audios", compute='_compute_slides_statistics', store=True)
    ...
    def _compute_slide_icon_class(self):
        icon_per_slide_type = {
            ...
            'local_audio': 'fa-file-audio-o',
            'local_video': 'fa-file-video-o',
            'youtube_video': 'fa-youtube-play',
            'google_drive_video': 'fa-play-circle-o',
            'vimeo_video': 'fa-vimeo',
            'cloudflare_stream_video': 'fa-cloud',
            'external_video': 'fa-external-link',
        }
        for slide in self:
    ...
    def _compute_slide_type(self):
        ...
        for slide in self:
            ...
            elif slide.slide_category == 'audio' and slide.source_type == 'local_file' and slide.binary_content:
                slide.slide_type = 'local_audio'
            elif slide.slide_category == 'video' and slide.source_type == 'local_file' and slide.binary_content:
            ...
            elif slide.slide_category == 'video' and slide.video_source_type == 'external':
                slide.slide_type = 'external_video'
            elif slide.slide_category == 'video' and slide.video_source_type == 'cloudflare_stream':
                slide.slide_type = 'cloudflare_stream_video'
            else:
                slide.slide_type = False
    ...
    @api.depends('slide_category', 'source_type', 'binary_content', 'google_drive_id', 'video_url', 'video_source_type', 'youtube_id', 'vimeo_id')
    def _compute_embed_code(self):
        ...
        for slide in self:
            ...
            if slide.slide_category == 'audio' and slide.source_type == 'local_file' and slide.binary_content:
                audio_url = f"{base_url}/web/content/slide.slide/{slide.id}/binary_content?download=false"
                embed_code = Markup(
                    '<audio controls="controls" preload="metadata" class="w-100" aria-label="%s">'
                    '<source src="%s"></source>'
                    '</audio>'
                ) % (_('Uploaded Audio'), audio_url)
            elif slide.slide_category == 'video':
                if slide.source_type == 'local_file' and slide.binary_content:
                    video_url = f"{base_url}/web/content/slide.slide/{slide.id}/binary_content?download=false"
                    base_video_tag = Markup(
                        '<video preload="metadata" playsinline '
                        'class="w-100 h-100 o_wslides_clean_video" aria-label="%s">'
                        '<source src="%s"></source>'
                        '</video>'
                    )
                    embed_code = base_video_tag % (_('Uploaded Video'), video_url)
                elif slide.video_source_type == 'youtube':
                    query_params = urls.url_parse(slide.video_url).query
                    query_params = query_params + '&theme=light' if query_params else 'theme=light'
                    query_params += '&rel=0&modestbranding=1&iv_load_policy=3&playsinline=1'
                ...
                elif slide.video_source_type == 'external':
                    direct_video_match = slide.video_url and re.search(
                        r'\.(?:mp4|webm|ogg)(?:[?#].*)?$', slide.video_url, re.IGNORECASE
                    )
                    if direct_video_match:
                        embed_code = Markup(
                            '<video controls="controls" preload="metadata" playsinline '
                            'class="w-100 h-100" aria-label="%s">'
                            '<source src="%s"></source>'
                            '</video>'
                        ) % (_('External Video'), escape(slide.video_url))
                    else:
                        embed_code = Markup(
                            '<iframe src="%s" allowFullScreen="true" frameborder="0" '
                            'allow="autoplay; fullscreen; picture-in-picture" aria-label="%s"></iframe>'
                        ) % (escape(slide.video_url), _('External Video'))
                elif slide.video_source_type == 'cloudflare_stream':
                    embed_code = Markup(
                        '<iframe src="%s" allowFullScreen="true" frameborder="0" '
                        'allow="autoplay; fullscreen; picture-in-picture" aria-label="%s"></iframe>'
                    ) % (escape(slide.video_url), _('Cloudflare Stream'))
    ...
    def _compute_video_source_type(self):
        for slide in self:
            ...
            if not video_source_type and slide.video_url and re.match(
                r'^https?://(?:iframe\.videodelivery\.net|customer-[a-z0-9]+\.cloudflarestream\.com)/.+$',
                slide.video_url,
            ):
                video_source_type = 'cloudflare_stream'
            if not video_source_type and slide.video_url and re.match(r'^https?://', slide.video_url):
    ...
    def _on_change_slide_category(self):
        ...
        elif self.slide_category not in ['document', 'video', 'audio'] and self.document_binary_content:
    ...
    def _fetch_external_metadata(self, image_url_only=False):
        ...
        slide_metadata = {}
        error = False
        if self.slide_category == 'video' and self.video_source_type == 'external':
            return {'slide_type': 'external_video'}, False
