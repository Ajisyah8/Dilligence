...
export class SlideUploadCategory extends Component {
    ...
    static sourceSettings = {
        document: {
            ...
            urlInputLabel: _t("External Link"),
        ...
        audio: {
            selectFileLabel: _t("Choose an Audio"),
            acceptedFiles: "audio/mpeg,audio/mp3,audio/wav,audio/ogg,audio/webm",
        },
    };
    ...
    async onChangeFileInput(ev) {
        ...
        if (!isImage && !isVideo && !isAudio && this.file.type !== "application/pdf") {
            this._alertDisplay(_t("Invalid file type. Please select a pdf, image, video, or audio file"));
            this._fileReset();
            this.state.preview.show = false;
            return;
        }
        ...
        if (file.type !== "application/pdf") {
            ...
            this.state.preview.show = isImage || isVideo || isAudio;
        } else {
            this.canSubmitForm = false;
            const dataURL = await getDataURLFromFile(file);
            this.file.data = dataURL.split(",", 2)[1];
    ...
    async _formValidateGetValues(forcePublished) {
        const sourceType = this.state.form.isLocalSource ? "local_file" : "external";
