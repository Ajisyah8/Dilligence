...
class WebsiteSlides(WebsiteProfile):
    ...
    def slide_view(self, slide, **kwargs):
        ...
        if slide.can_self_mark_completed and not slide.user_has_completed \
           and slide.channel_id.channel_type == 'training' and slide.slide_category not in ['video', 'audio']:
    ...
    @http.route('/slides/slide/quiz/import_xlsx', type='jsonrpc', methods=['POST'], auth='user', website=True)
    def slide_quiz_import_xlsx(self, slide_id, file_content, file_name=None):
        fetch_res = self._fetch_slide(slide_id)
        if fetch_res.get('error'):
            return fetch_res
        ...
        if not slide.channel_id.can_upload:
            return {'error': _('You cannot import questions into this course content.')}

        if not file_content:
            return {'error': _('Please select an Excel file to import.')}

        try:
            binary_content = base64.b64decode(file_content)
        except Exception:
            return {'error': _('The uploaded file is not valid Base64 content.')}

        question_model = request.env['slide.question']
        try:
            question_values_list = question_model._prepare_questions_from_xlsx(slide, binary_content)
        except ValidationError as exc:
            return {'error': exc.args[0]}

        next_sequence = max(slide.question_ids.mapped('sequence') or [0]) + 1
        created_questions = request.env['slide.question']
        for question_values in question_values_list:
            answer_commands = [(0, 0, answer_values) for answer_values in question_values.pop('answer_ids')]
            question_values['sequence'] = next_sequence
            question_values['answer_ids'] = answer_commands
            next_sequence += 1
            created_questions |= question_model.create(question_values)
        ...
        return {
            ...
            'file_name': file_name,
            'questions': [
                request.env['ir.qweb']._render('website_slides.lesson_content_quiz_question', {
                    'slide': slide,
                    'question': question,
                })
    ...
    @http.route('/slides/slide/quiz/import_template', type='http', auth='user', website=True)
    def slide_quiz_import_template(self):
        import xlsxwriter  # noqa: PLC0415

        output = io.BytesIO()
        workbook = xlsxwriter.Workbook(output, {'in_memory': True})
        worksheet = workbook.add_worksheet('Quiz Import')

        headers = [
            'Question',
            'Answer 1', 'Correct 1', 'Comment 1',
            'Answer 2', 'Correct 2', 'Comment 2',
            'Answer 3', 'Correct 3', 'Comment 3',
            'Answer 4', 'Correct 4', 'Comment 4',
        ]
        example_row = [
            'What animal can fly?',
            'Bird', 'TRUE', 'Correct, birds can fly.',
            'Elephant', 'FALSE', 'Elephants cannot fly.',
            'Butterfly', 'TRUE', 'Correct, butterflies can fly too.',
            'Stone', 'FALSE', 'A stone is not an animal and cannot fly.',
        ]
        ...
        for column, value in enumerate(example_row):
            worksheet.write(1, column, value, text_style)
        ...
        return request.make_response(
            ...
            headers=[
                ...
                ('Content-Disposition', content_disposition('quiz_import_template.xlsx')),
            ],
        )
