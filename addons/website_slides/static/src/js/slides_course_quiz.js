    ...
    var Quiz = publicWidget.Widget.extend({
        ...
        events: {
            ...
            'click .o_wslides_js_quiz_import_trigger': '_onImportTriggerClick',
            'change .o_wslides_js_quiz_import_input': '_onImportFileChange',
        ...
        */
        ...
        _renderImportedQuestions: function (questionTemplates) {
            const $insertionPoint = this.$('.o_wslides_js_lesson_quiz_new_question');
            questionTemplates.forEach((questionTemplate) => {
                $insertionPoint.before(questionTemplate);
            });

            this.quiz.questionsCount += questionTemplates.length;
            this.$('.o_wslides_js_quiz_add_quiz').addClass('d-none');
            this.$('.o_wslides_js_quiz_add_question').removeClass('d-none');

            if (!this.$('.o_wslides_js_lesson_quiz_validation').length) {
                this.$('.o_wslides_js_lesson_quiz_new_question').after('<div class="o_wslides_js_lesson_quiz_validation pt-3"></div>');
                this._renderValidationInfo();
            }
        },

        _setImportMessage: function (message, level = 'info') {
            const $message = this.$('.o_wslides_js_quiz_import_message');
            if (!$message.length) {
                return;
            }
            $message
       ...
       _afterJoin: function () {
        ...
        _onImportTriggerClick: function (ev) {
            ev.preventDefault();
            const input = this.el.querySelector('.o_wslides_js_quiz_import_input');
            if (input) {
                input.value = '';
                input.click();
            }
        ...
        _onImportFileChange: async function (ev) {
            ...
            if (!file.name.toLowerCase().endsWith('.xlsx')) {
                this._setImportMessage(_t('Please select a .xlsx file.'), 'danger');
                input.value = '';
                return;
            }

            this._setImportMessage(_t('Importing questions from Excel...'), 'info');
            try {
                const fileContent = await this._readImportFile(file);
                const result = await rpc('/slides/slide/quiz/import_xlsx', {
                    slide_id: this.slide.id,
                    file_name: file.name,
                ...
                if (result.error) {
                    this._setImportMessage(result.error, 'danger');
                    return;
                }

                this._renderImportedQuestions(result.questions || []);
                this._setImportMessage(
                    _t('%s question(s) imported successfully.').replace('%s', result.count),
                    'success',
                );
            } catch (error) {
                this._setImportMessage(_t('The Excel file could not be imported.'), 'danger');
            } finally {
                input.value = '';
            }
        },
