...
class WebsiteSlidesSurvey(WebsiteSlides):
    ...
    def _get_certification_survey_for_management(self, survey_id):
        survey = request.env['survey.survey'].browse(int(survey_id)).exists()
        ...
        survey.check_access('write')
        survey.check_access_rule('write')
        return survey

    @http.route(['/slides_survey/certification/manage_questions'], type='http', auth='user', website=True)
    def certification_manage_questions(self, survey_id, **kw):
        survey = self._get_certification_survey_for_management(survey_id)
        return request.redirect(f'/odoo/survey.survey/{survey.id}')

    @http.route(['/slides_survey/certification/import_questions'], type='http', auth='user', website=True)
    def certification_import_questions(self, survey_id, **kw):
        survey = self._get_certification_survey_for_management(survey_id)
        wizard = request.env['survey.question.import.wizard'].create({
            'survey_id': survey.id,
        })
        action = request.env.ref('website_slides_survey.action_survey_question_import_wizard')
        return request.redirect(f'/odoo/action-{action.id}/{wizard.id}')

    @http.route(['/slides_survey/certification/import_template'], type='http', auth='user', website=True)
    def certification_import_template(self, **kw):
        import xlsxwriter  # noqa: PLC0415

        output = io.BytesIO()
        workbook = xlsxwriter.Workbook(output, {'in_memory': True})
        worksheet = workbook.add_worksheet('Certification Import')

        headers = [
            'Section', 'Title', 'Question Type', 'Description', 'Mandatory',
            'Answer 1', 'Correct 1', 'Score 1',
            'Answer 2', 'Correct 2', 'Score 2',
            'Answer 3', 'Correct 3', 'Score 3',
            'Correct Value', 'Score',
            'Scale Min', 'Scale Max', 'Scale Min Label', 'Scale Mid Label', 'Scale Max Label',
            'Placeholder', 'Validation Email',
        ]
        example_rows = [
            ['Basics', 'What can fly?', 'simple_choice', 'Pick one answer.', 'TRUE',
             'Bird', 'TRUE', 1, 'Elephant', 'FALSE', 0, 'Stone', 'FALSE', 0,
             '', '', '', '', '', '', '', '', ''],
            ['Basics', 'Select all mammals', 'multiple_choice', '', 'TRUE',
             'Whale', 'TRUE', 1, 'Shark', 'FALSE', 0, 'Dog', 'TRUE', 1,
             '', '', '', '', '', '', '', '', ''],
            ['Basics', 'How many continents are there?', 'numerical_box', '', 'TRUE',
             '', '', '', '', '', '', '', '', '',
             7, 1, '', '', '', '', '', '', ''],
            ['Profile', 'Your support email', 'char_box', '', 'FALSE',
             '', '', '', '', '', '', '', '', '',
             '', '', '', '', '', '', '', 'name@example.com', 'TRUE'],
            ['Feedback', 'Rate the training', 'scale', '', 'FALSE',
             '', '', '', '', '', '', '', '', '',
             '', '', 1, 5, 'Poor', 'Average', 'Excellent', '', ''],
        ]

        header_style = workbook.add_format({'bold': True, 'bg_color': '#D9EAF7', 'border': 1})
        text_style = workbook.add_format({'text_wrap': True, 'valign': 'top'})

        for column, header in enumerate(headers):
            worksheet.write(0, column, header, header_style)
            worksheet.set_column(column, column, 18 if column not in {3} else 28)

        for row_index, row_values in enumerate(example_rows, start=1):
            for column, value in enumerate(row_values):
                worksheet.write(row_index, column, value, text_style)
        ...
        return request.make_response(
            ...
            headers=[
                ...
                ('Content-Disposition', content_disposition('certification_question_import_template.xlsx')),
            ],
        )
